...
function rewardPerks(){const p=read(REWARD_PERKS,{vaultFree:0,vaultDiscount:0,vaultDiscountUses:0,vaultDiscountQueue:[]});p.vaultFree=Math.max(0,Number(p.vaultFree||0));p.vaultDiscount=Math.max(0,Number(p.vaultDiscount||0));p.vaultDiscountUses=Math.max(0,Number(p.vaultDiscountUses||0));p.vaultDiscountQueue=Array.isArray(p.vaultDiscountQueue)?p.vaultDiscountQueue.filter(x=>Number(x)>0).map(Number):[];if(!p.vaultDiscountQueue.length&&p.vaultDiscountUses>0&&p.vaultDiscount>0){for(let i=0;i<p.vaultDiscountUses;i++)p.vaultDiscountQueue.push(p.vaultDiscount)}if(p.vaultDiscountQueue.length){p.vaultDiscount=p.vaultDiscountQueue[0];p.vaultDiscountUses=p.vaultDiscountQueue.length}return p}
...
function grant(i,price,serverAccepted=false){const s=shelf();if(s.owned[i.id])return true;price=Math.max(0,Math.floor(Number(price)||0));const vd=Number(localStorage.getItem("lizzyVipShelfDiscount")||0),rp=rewardPerks(),rd=Number((rp.vaultDiscountQueue&&rp.vaultDiscountQueue[0])||localStorage.getItem("lizzyRareShelfDiscount")||0),discount=Math.max(vd,rd),originalPrice=price;if(discount>0&&price>0){price=Math.max(1,Math.ceil(price*(100-discount)/100));const useReward=rd>=vd&&rd>0;if(useReward){const p=rewardPerks();if(p.vaultDiscountQueue.length)p.vaultDiscountQueue.shift();else p.vaultDiscountUses=Math.max(0,Number(p.vaultDiscountUses||0)-1);p.vaultDiscountUses=p.vaultDiscountQueue.length||p.vaultDiscountUses;p.vaultDiscount=Number(p.vaultDiscountQueue[0]||0);if(p.vaultDiscountUses>0&&p.vaultDiscount>0)localStorage.setItem("lizzyRareShelfDiscount",String(p.vaultDiscount));else localStorage.removeItem("lizzyRareShelfDiscount");saveRewardPerks(p)}else if(vd>0){localStorage.removeItem("lizzyVipShelfDiscount")}window.dispatchEvent(new Event("lizzyRewardPerksUpdated"))}if(!serverAccepted&&(price<1||wallet()<price))return false;if(price>0)setWallet(Math.max(0,wallet()-price));s.owned[i.id]={at:new Date().toISOString(),price,originalPrice,discount};saveShelf(s);if(i.kind==="letter"){let l=read(LETTERS,[]);if(!Array.isArray(l))l=[];const entry={id:i.id,title:i.publicName,content:i.content,folder:"open_when_purchased",purchasedAt:new Date().toISOString()};const at=l.findIndex(x=>x.id===i.id);if(at>=0)l[at]={...l[at],...entry};else l.push(entry);write(LETTERS,l);renderLetters();window.dispatchEvent(new Event("lizzyPurchasedLettersUpdated"))}if(i.kind==="dossier")addDossier(i,price);if(i.kind==="document"&&i.id==="archive_x17"){addDossier({id:"archive_x17",title:"Cody Legal Documents",content:i.content},price)}if(i.kind==="mikael_token"){const t=read(MTOKENS,{inventory:{},history:[]});t.inventory=t.inventory||{};t.history=t.history||[];t.inventory["UNO Reverse"]=Number(t.inventory["UNO Reverse"]||0)+1;t.history.push({type:"earned",token:"UNO Reverse",source:"Mystery Reward",at:new Date().toISOString()});write(MTOKENS,t);renderMikaelTokens()}window.dispatchEvent(new Event("lizzyStoreRefresh"));return true}
